...
The WebRTC Camera media player is a playback surface. If your WebRTC Camera version supports `volume_entity`, point it at VIGI Control's speaker volume number entity so one media player can handle both playback and Home Assistant's `media_player.volume_set` calls. VIGI Control exposes camera speaker volume numbers as `0..100%`, matching Home Assistant's normal percentage display while WebRTC maps media-player `volume_level` values from `0..1` onto that native range.
